...
void main() async {
  ...
  group('Adding new measurement sessions works correctly', () {
    ...
    test(
        ...
        () {
      expect(measurements.measurements[2].id, '12345');
      expect(measurements.measurements[2].clientId, 'client001');
      expect(measurements.measurements[2].date, DateTime(2020, 10, 24));
    });

    test('Added measurement session\'s measurements list has proper length',
        () {
      expect(measurements.measurements[2].measurements.length, 3);
    });
    test('First measurement of added session has all parameters set properly',
        () {
      final firstMeasurement = measurements.measurements[2].measurements[0];
